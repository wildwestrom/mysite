...
(def portfolio
  [{:group-name "Finished projects"
    :projects   [{:name  "Feedback Box Component"
                  :desc  ["A component designed to allow users to send feedback to an endpoint. It was designed to be easily deployable as a single " [:code ".js"] " file. Despite having no prior knowledge I wrote this using the " [common/generic-link "https://github.com/tonsky/rum" "Rum"] " library. This was my first paid commission."]
                  :links [{:title "Source"
   ...
   {:group-name "In Progress"
    :projects   [{:name  "Death Calendar"
                  ...
                  :links [{:title "Source"
                           ...
                           :icon  faGithub}]}
                 {:name  "TS Update"
                  :desc  ["A simple server designed to generate an RSS feed by scraping the page of a site I have a membership to. The page is currently undergoing updates, and the implementation is quite inefficient, so it probably doesn't work at the moment."]
                  :links [{:title "Source"
                           :icon  faGithub
                           :href "https://github.com/wildwestrom/ts-update"}]}]}])
